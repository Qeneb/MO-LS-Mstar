...
class MoAstarMAPFBase:
  ...
  def GetCost(self, loc, nloc):
    ...
    for idx in range(self.num_robots): # loop over all robots
      if nloc[idx] != loc[idx]: # there is a move, not wait in place.
        out_cost = out_cost + self.edge_cost_dict[(loc,nloc)]
      else: # robot stay in place.
        if len(self.s_f.loc) == 0: # policy mode, no goal specified.
          out_cost = out_cost # stay-in-place fixed cost
        elif loc[idx] != self.s_f.loc[idx]: # nloc[idx] = loc[idx] != s_f.loc[idx], robot not reach goal.
          out_cost = out_cost # np.ones((self.cdim)) # stay in place, fixed energy cost for every robot
    return out_cost
  ...
  def GetNeighbors(self, s, tstart):
    ...
    for idx in range(self.num_robots): # loop over all robot
      ...
      cx = int(np.floor(s.loc[idx]/self.nxt)) # current x
      cy = int(s.loc[idx]%self.nxt) # current y
      ...
      for action_idx in range(len(tmp_action_set_x)):
        ...
        if not (ny < 0 or ny >= self.nxt or nx < 0 or nx >= self.nyt):
          # do not exceed border
          nloc = nx*self.nxt + ny
          loc = s.loc[idx]
          if self.edge_cost_dict[(loc,nloc)][0] < 99999: # not obstacle
            nid_dict[idx].append( nx*self.nxt+ny )
    ...
    for idx in range(self.num_robots): # loop over all robots
      if nloc[idx] != loc[idx]: # there is a move, not wait in place.
        if len(self.cost_grids) > 0 and len(self.cost_grids) >= self.cdim :
          ...
          out_cost = self.edge_cost_dict[(loc[idx],nloc[idx])]
          # for ic in range(self.cdim):
          #   out_cost[ic] = out_cost[ic] + self.cost_vecs[idx][ic]*self.cost_grids[ic][cy,cx]
        else:
          out_cost = out_cost + self.edge_cost_dict[(loc[idx],nloc[idx])]
      else: # robot stay in place.
